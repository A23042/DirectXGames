...
namespace {
	const int distQNum = 2;	// âÊñ ï™äÑç≈ëÂêî
	struct UpdateObject {
		GameObject* object;
		bool initialized;
		bool destroyMe;
	...
	struct DrawObject {
		GameObject* object;
		int order;
		float distQ[distQNum];	// éãì_Ç©ÇÁÇÃãóó£ÇÃÇQèÊ[âÊñ ï™äÑç≈ëÂêî]
		bool visible;
	...
	bool needSortUpdate;
	bool needSortDraw;
	int drawTimes = 2;	// ï`âÊâÒêî(âÊñ ï™äÑêî)
	int drawCounter;	// ï`âÊÉJÉEÉìÉ^Å[(âΩâÊñ ñ⁄ÇÃï`âÊÇ©)
};
...
{
	for(drawCounter = 0; drawCounter < drawTimes; drawCounter++)	// drawTimesâÒêîï`âÊèàóùÇçsÇ§
	{
		if (needSortDraw) {
